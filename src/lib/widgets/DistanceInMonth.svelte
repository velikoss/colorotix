    ...
    function getData() {
        ...
        for (let month = 1; month <= 12; month++) {
            ...
            aggregated[monthName] =
                yearData[monthKey]?.distance_in_month.toFixed(2) || 0;
        }
...
<MonthYearPickerWide data={fuel_km_per_month} bind:currentMonth bind:currentYear showMonthPicker={false}>
    <div class="w-full h-full flex flex-col relative">
        <div class="absolute text-2xl text-center w-full">Динамика проеденного расстояния</div>
        <Bars
            margins={{ top: 50, right: 20, bottom: 30, left: 90 }}
            rawData={getData()}
            valueKeys={["distance_in_month"]}
        />
    </div>
</MonthYearPickerWide>